...
CNode* FindNode(std::string addrName, bool bchkIp)
{
    LOCK(cs_vNodes);
    BOOST_FOREACH(CNode* pnode, vNodes)
	{
		if( addrName.size() )
		{
			if( bchkIp )
			{
				string s = pnode->addr.ToStringIP().c_str();
				unsigned int i = addrName.find(s.c_str());
				if(  i == 0 )
				{
					return (pnode);
				}
...
bool AddOrRemoveNode(string strNd)
{
	...
	try
	{
		if( strNd.size() > 6 )
		{
		bool bConnTo = true, bAdd = false, bReconn=false;
		...
		if( dEc == 0 )
		{
			...
			if( dEc == 0 )
			{
				...
				if( dEc == 0 )
				{
					bConnTo = false;
					strNode = strNode.substr(dEc + 1);
					dEc = vDisconnectedIp.find(strNode.c_str());
					if( dEc == string::npos )
					{
						vDisconnectedIp = vDisconnectedIp + ";" + strNode.c_str();
					}
				}
		...
		if( bConnTo )
		{
			...
			if( pnode == NULL )
			{
				...
				else if( (pnode->fDisconnect) || (pnode->hSocket == INVALID_SOCKET) )
					bConn = true;
			}
			if( bConn )
			{
...
CNode* ConnectNode(CAddress addrConnect, const char *pszDest)
{
	...
	if( vDisconnectedIp.size() > 2 )
	{
		string shIp = addrConnect.ToStringIP().c_str();
		unsigned int dEc = vDisconnectedIp.find(shIp.c_str());
		if( dEc != string::npos )
		{
			printf("%s) Disconnect\n", shIp.c_str());
			return NULL;
    ...
    loop
    {
        ...
        if (hListenSocket != INVALID_SOCKET && FD_ISSET(hListenSocket, &fdsetRecv))
        {
            ...
            else
            {
				...
				if( vDisconnectedIp.size() > 2 )
				{
					string shIp = addr.ToStringIP().c_str();
					unsigned int dEc = vDisconnectedIp.find(shIp.c_str());
					if( dEc != string::npos )
					{
						closesocket(hSocket);
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Hotcoin " + FormatFullVersion();
...
bool OpenNetworkConnection(const CAddress& addrConnect, CSemaphoreGrant *grantOutbound, const char *strDest, bool fOneShot)
{
    ...
    if (!strDest)
        if (IsLocal(addrConnect) ||
            ...
            FindNode(addrConnect.ToStringIPPort().c_str(), false))
            return false;
    if (strDest && FindNode(strDest, false))
        return false;
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Hotcoin is probably already running."), addrBind.ToString().c_str());
        else
...
void RelayTransaction2(const CMerkleTx& tx, const uint256& hash)
{
    CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
    ss.reserve(10000);
    ss << tx;
    RelayTransaction2(tx, hash, ss);
}

void RelayTransaction2(const CMerkleTx& tx, const uint256& hash, const CDataStream& ss)
{
    CInv inv(MSG_TX, hash);
    {
        LOCK(cs_mapRelay);
