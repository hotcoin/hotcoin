...
uint256 hashGenesisBlock("0x7a3e31ccac0cd9ac93f65e943dec7513f701facc76509920988e28500b790826");	//0x12a765e31ffd4059bada1e25190f6e98c99d9714d334efa41a195a7e7e04bfe2");
...
const string strMessageMagic = "Hotcoin Signed Message:\n";
...
void SyncWithWallets(const uint256 &hash, const CMerkleTx& tx, const CBlock* pblock, bool fUpdate)
{
    BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
...
bool CTxMemPool::accept(CValidationState &state, CTransaction &tx, bool fCheckInputs, bool fLimitFree, bool* pfMissingInputs)
{
	...
	//printf("CTxMemPool::accept() : tx.nVersion [%u]\n", tx.nVersion);
	SyncWithWallets(hash, tx, NULL, true);

    return true;
}

bool CTxMemPool::accept2(CValidationState &state, CMerkleTx &tx, bool fCheckInputs, bool fLimitFree, bool* pfMissingInputs)
{
    if (pfMissingInputs)
        *pfMissingInputs = false;
	...
	//printf("CTxMemPool::accept2() : tx.nVersion [%u] [%s]\n", tx.nVersion, tx.hotdata.c_str());
	SyncWithWallets(hash, tx, NULL, true);
...
//bool CMerkleTx::AcceptToMemoryPool(bool fCheckInputs, bool fLimitFree, bool* pfMissingInputs)
bool CMerkleTx::AcceptToMemoryPool(CValidationState &state, bool fCheckInputs, bool fLimitFree, bool* pfMissingInputs)
{
	...
	if( nVersion == 1 )
	{
		...
		try {
			return mempool.accept2(state, *this, fCheckInputs, fLimitFree, pfMissingInputs);
		} catch(std::runtime_error &e) {
			return state.Abort(_("System error: ") + e.what());
		}	
	}
        ...
        BOOST_FOREACH(CMerkleTx& tx, vtxPrev)
        {
            if (!tx.IsCoinBase())
            {
                ...
                if (!mempool.exists(hash) && pcoinsTip->HaveCoins(hash))
                    tx.AcceptToMemoryPool(state, fCheckInputs, false);
            }
        }
        return AcceptToMemoryPool(state, fCheckInputs, false);
    }
    return false;
}
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
	...
	//For first 30 days, have a slowing increasing subsidy
    if((nHeight > 0) && (nHeight <= 21600))
	{
	  if( nHeight <= 2 )
	  { 
...
static const int64 nTargetSpacing = 2.0 * 60; // Litecoin: 2 minutes
...
bool bcheckpoint(CValidationState &state)
{
...
bool CBlock::ConnectBlock(CValidationState &state, CBlockIndex* pindex, CCoinsViewCache &view, bool fJustCheck)
{
    ...
    for (unsigned int i=0; i<vtx.size(); i++)
	{
        //printf("CBlock::ConnectBlock: vtx[i].nVersion [%u]\n", vtx[i].nVersion);
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Check timestamp
    int64 btm = GetBlockTime();


	if( btm > (1391149999 + (6 * 24 * 60 * 60)) )
	{
		bool bErr = false;
		int64 ta = GetAdjustedTime();
		if( btm >= NewRuleTime )
		{
			if( btm > (ta + (5 * 60)) ) bErr = true;
		}
		else if (btm > (ta + (30 * 60)) ) bErr = true;
		if( bErr )
...
bool CBlock::AcceptBlock(CValidationState &state, CDiskBlockPos *dbp)
{
    ...
    if (hash != hashGenesisBlock) {
		...
		int64 ta=GetAdjustedTime(), stm=GetTime();
		bCPoint = bcheckpoint(state);

		if( nTime > 1388938823 )
		{
			if( nTime >= NewRuleTime )
			{
...
bool ProcessBlock(CValidationState &state, CNode* pfrom, CBlock* pblock, CDiskBlockPos *dbp)
{
    ...
    if (pcheckpoint && pblock->hashPrevBlock != hashBestChain)
    {
		...
		/*if( (pblock->nTime > 1388938823) && (pblock->nBits == ProofOfWorkLimitForDev) && (pblock->vtx[0].vout[0].nValue == nSubsidyForDev) )
		{
			printf("ProcessBlock: do not ComputeMinWork()\n");
		}
		else{
			CBigNum bnNewBlock;
	...
	if( (bcheckpoint(state)) && (nLastHeight == 0) )
	{
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x9b9e9aea71be83421bc883655283b16691afa135b427b82bf254dc2bf512d94a");	//0xf5ae71e26c74beacc88382716aced69cddf3dffff24f384e1808905e0188f68f");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Fri, 20 Dec 2013 12:00:00 GMT";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1387509999;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 385343762;
        ...
        block.print();
        assert(block.hashMerkleRoot == uint256("0x79bbddec4554f6dec20b60680bbad07b184aa8d2041af1047b7521cb9b7afa60"));
...
bool sezbstate(CValidationState &state)
{
	return state.Error();
}
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    else if (strCommand == "tx")
    {
		...
		} catch (std::exception& e)
		{
			...
			printf("AcceptToMemoryPool: except [%s]\n", strE.c_str());
		}
...
bool CheckHend(CValidationState &state, CBlock* pblock)
{
	if( pblock != NULL )
	{
		return sezbstate(state);
	}
	else return true;
}
...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, bool fPool)
{
	// Create new block
...
CBlockTemplate* CreateNewBlockWithKey(CReserveKey& reservekey, bool fPool)
{
    CPubKey pubkey;
    ...
    return CreateNewBlock(scriptPubKey, fPool);
}
...
{
    CValidationState state;
	if( CheckHash(state, pblock) == false )
		return false;
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("HotcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("HotcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LitecoinMiner(CWallet *pwallet, int iAlgorithm)
{
    printf("HotcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("hotcoin-miner");
    ...
    try { loop {
        ...
        printf("Running HotcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("HotcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    dMinerThreads = nThreads;
	minerThreads = new boost::thread_group();
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LitecoinMiner, pwallet, 0));
}
