...
void ThreadIRCSeed()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #hotcoinTEST3\r");
            Send(hSocket, "WHO #hotcoinTEST3\r");
        } else {
            // randomly join #M3GAC01N00-#M3GAC01N99
            int channel_number = GetRandInt(100);
	 channel_number = 0; // For now, just use one channel
            Send(hSocket, strprintf("JOIN #hotcoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #hotcoin%02d\r", channel_number).c_str());
        }
