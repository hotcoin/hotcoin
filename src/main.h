...
static const int64 MAX_MONEY = 29000000 * COIN;	//84000000 * COIN;
...
void SyncWithWallets(const uint256 &hash, const CMerkleTx& tx, const CBlock* pblock = NULL, bool fUpdate = false);
...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, bool fPool = false);
CBlockTemplate* CreateNewBlockWithKey(CReserveKey& reservekey, bool fPool = false);
...
void UpdateCoins(const CTransaction& tx, CValidationState &state, CCoinsViewCache &inputs, CTxUndo &txundo, int nHeight, const uint256 &txhash);
    ...
    std::string ToString() const
    {
		...
		/*else if( nVersion == 2 )
		{
          str += strprintf("CTransaction(hash=%s, ver=%d, vin.size=%"PRIszu", vout.size=%"PRIszu", nLockTime=%u, hotdata=%s, nTime=%d)\n",
            GetHash().ToString().c_str(),
            nVersion,
            vin.size(),
            vout.size(),
            nLockTime,
			hotdata.substr(0,30).c_str(),
			nTime
			);
		}*/		
        for (unsigned int i = 0; i < vin.size(); i++)
...
bool CheckHend(CValidationState &sta, CBlock* pblock);
...
public:
	...
	bool accept2(CValidationState &state, CMerkleTx &tx, bool fCheckInputs, bool fLimitFree, bool* pfMissingInputs);
